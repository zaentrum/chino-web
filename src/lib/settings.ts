...
export interface ChinoSettings {
  ...
  // Default audio language. The player picks the closest matching
  // audio track on each item; 'orig' = whatever TMDB lists as the
  // item's `original_language`. A manual switch in the audio menu
  // overrides for the current playback only — next item reverts to
  // this preference.
  audio: {
    preferredLang: string; // 'eng' / 'deu' / … / 'orig'
  };
}
