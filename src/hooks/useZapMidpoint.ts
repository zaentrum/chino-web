...
/** Fixed seek ratio for the DETERMINISTIC first Zap card. Unlike every
 *  other card (which re-rolls a fresh random ratio per mount for the
 *  channel-flip feel), card[0] must land on the SAME bytes in two
 *  independent places: the app-start warm on the home screen and the
 *  Zap screen's first card. A fixed ratio is the only way those two
 *  warm-then-play the exact same init+segment window. 0.35 ≈ early act
 *  2 — far enough past the opening logo to feel mid-scene, not so deep
 *  that short content runs out of runway. */
export const FIRST_CARD_RATIO = 0.35;

/**
 * Deterministic seek for the FIRST Zap card, shared by the app-start
 * warm and the Zap screen so both warm/play the identical segment
 * window. Honours a server-provided seekSec verbatim (the warm pool
 * already rolled + pre-warmed those bytes); otherwise derives a stable
 * seek from duration at FIRST_CARD_RATIO — no Math.random(), so it is
 * byte-for-byte reproducible across the two call sites.
 *
 * Returns null when we can't pick a usable mid-content seek (too-short
 * content → 'fallback'); callers should treat that as "skip warming".
 */
export function firstCardSeekSec(item: {
  duration_ms?: number;
  __zapSeekSec?: number;
}): number | null {
  if (typeof item.__zapSeekSec === 'number' && item.__zapSeekSec > 0) {
    return item.__zapSeekSec;
  }
  const mid = pickZapMidpoint({ durationMs: item.duration_ms, randomRatio: FIRST_CARD_RATIO });
  if (mid.source === 'fallback' || mid.seekSec <= 0) return null;
  return mid.seekSec;
}

/**
 * Stamp a deterministic `__zapSeekSec` (+ matching `__zapMidSource`)
 * onto the FIRST Zap card so the app-start warm, the byte prefetch, and
 * ZapCard all read the same seek from the same field. ZapCard and
 * buildTarget already short-circuit their own roll when `__zapSeekSec`
 * is present, so stamping is the whole mechanism — no extra cross-screen
 * holder needed.
 *
 * - If a seek is already stamped (server warm pool), the item is
 *   returned unchanged (we never clobber a server-rolled seek).
 * - If we can't derive a usable seek, the item is returned unchanged so
 *   the screen falls back to its normal per-mount behaviour.
 */
export function stampFirstCardSeek<T extends { duration_ms?: number; __zapSeekSec?: number; __zapMidSource?: string }>(
  item: T,
): T {
  if (typeof item.__zapSeekSec === 'number' && item.__zapSeekSec > 0) return item;
  const seekSec = firstCardSeekSec(item);
  if (seekSec === null) return item;
  return { ...item, __zapSeekSec: seekSec, __zapMidSource: 'percent' };
}
