...
export interface WatchHistoryEntry extends KatalogItem {
  ...
  // Episode coordinates — present when type === 'episode'. chino-api
  // resolves each history row against katalog, so these ride on the
  // same payload (katalog.Item carries them); no extra fetch needed
  // to render the SxxExx label. parent_id points at the series, whose
  // title is NOT inlined here — ProfilePage resolves it separately.
