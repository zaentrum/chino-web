...
interface Subtitle {
  ...
  // pre-date the format column being surfaced).
  format?: string;
}
...
export function PlayerPage({ itemId }: PlayerPageProps) {
  ...
  // own listener; we just give it the canvas + sub URL and tear down
  // when the track changes. The canvas is keyed per active PGS sub id in
  // JSX (see render), so each renderer binds to a fresh, untransferred
  // node — libpgs's worker mode calls transferControlToOffscreen(),
  // which is one-way, so a canvas can never be reused across renderers.
  // This ref just points at whichever keyed canvas is currently mounted.
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/items/${itemId}/subtitles`, {
      ...
      .then((j) => {
        // Web renderers we actually have: native <track> for text
        // (webvtt/srt/missing format) and libpgs-js for PGS. Bitmap
        // sidecar formats with NO web renderer (vobsub/dvdsub, dvb)
        // would mount as broken <track> elements (silent cue-load
        // failure) or be unrenderable — drop them from the selectable
        // list so the user never picks an unplayable track.
        const NO_WEB_RENDERER = new Set([
        ...
        const list = (j?.subtitles ?? [])
          ...
          .map((s) => ({
            ...
            // so the libpgs renderer isn't recreated on an already-transferred
            // canvas (InvalidStateError) and a 15MB .sup isn't re-downloaded
            // every ~5 min. Same fix the TV/segments use.
            url: s.url.includes('?') ? `${s.url}&stream=${encodeURIComponent(streamToken ?? '')}` : `${s.url}?stream=${encodeURIComponent(streamToken ?? '')}`,
          }));
  ...
  const mergedSubs = useMemo<Subtitle[]>(() => {
    ...
    if (info?.subtitle_tracks && streamToken) {
      ...
      for (const t of info.subtitle_tracks) {
        ...
        // A track without a stream index can't be addressed — skip it
        // instead of requesting /subtitles/undefined.vtt (seen as 400s
        // in the chino-stream access log).
  ...
  // PGS sidecar rendering. libpgs-js owns one renderer per track URL;
  // teardown + recreate when the active PGS subtitle changes (or when
  // no PGS sub is active). The library binds to videoRef's timeupdate
  // / seeked listeners internally, so currentTime sync is automatic.
  //
  // Only a single PGS track can render at a time — libpgs's overlay
  // covers the whole canvas. If the user has selected two PGS subs
  // (rare), we render activeSubIds[0]; the second goes invisible.
  // Mixed VTT-on-top-of-PGS dual-tracking still works (VTT goes via
  // native <track>, PGS goes via canvas, they composite).
  const activePgsSub = useMemo<Subtitle | null>(() => {
    for (const id of activeSubIds) {
      const s = mergedSubs.find((m) => m.id === id);
      if (s?.format === 'pgs') return s;
    }
    return null;
  }, [activeSubIds, mergedSubs]);
  ...
  useEffect(() => {
    ...
    // No PGS sub → the keyed canvas is unmounted; nothing to draw. A
    // freshly-keyed canvas mounts blank for the next track, so there's
    // no stale frame to wipe (and calling getContext('2d') on an
    // already-transferred canvas would throw InvalidStateError).
    if (!v || !canvas || !activePgsSub) return;

    // Defensively dispose any prior renderer (terminating its worker)
    // before creating the next, so only one renderer is ever alive.
    // The canvas key change already mounts a fresh, untransferred node;
    // disposing here guards against an overlapping effect run (e.g.
    // StrictMode double-invoke or a rapid token-driven re-fire).
    ...
    return () => {
      r.dispose();
      if (pgsRendererRef.current === r) pgsRendererRef.current = null;
    };
  ...
  const filePlayerReport = (tech: string, errName: string, message: string) => {
    ...
    void fileAutoReport({
      ...
      errorName: errName,
      message,
      title: `Playback failed: ${message}`.slice(0, 120),
      description: tech,
      extraContext: {
        itemId,
        mode: effectiveMode,
    ...
    >
      ...
      >
        ...
        {activeSubIds.map((id) => {
          const s = mergedSubs.find((m) => m.id === id);
          if (!s) return null;
          // Bitmap formats (PGS) are rendered via libpgs-js onto
          // the canvas sibling below — <track> can only carry text
          // formats. Listing a .sup as a <track src> would have
          // the browser silently fail to mount the cue list.
          if (s.format === 'pgs') return null;
          return (
            <track
              key={`${s.id}|${s.url}`}
              id={s.id}
      ...
      {/* Sibling canvas for image subtitle rendering. libpgs-js
          ...
          canvas.transferControlToOffscreen(), a ONE-WAY transfer —
          once a canvas is handed to a worker it can never be reused.
          Re-keying forces React to discard the transferred node and
          mount a fresh, untransferred canvas for every renderer, so
          switching PGS tracks / toggling Off / token rotation /
          StrictMode double-invoke can't hit InvalidStateError. The
          canvas is only mounted when a PGS sub is active. */}
      {activePgsSub && (
        <canvas
...
// detector evidence (intro/credit markers, chromaprint xN, silence N.Ns,
