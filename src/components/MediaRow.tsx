...
interface Media {
  ...
  // Passed straight through to MediaCard. The CW rail wires the remove
  // callback; rails that hide watched titles set dropWhenWatched so a
  // Mark-as-watched toggle collapses the card out of the row.
...
interface MediaRowProps {
  ...
  onSeeAll?: () => void;
  /**
   * Makes the row title itself a click target (the Watchlist hub's
   * shelf headers open the per-list view). Omit for a plain heading —
   * the home rails keep their non-interactive titles.
   */
  onTitleClick?: () => void;
  /**
 ...
 */
export function MediaRow({ title, items, onSeeAll, onTitleClick, noLoop = false }: MediaRowProps) {
