...
import { useZapAppStartWarm } from '../../hooks/useZapPrefetch';
...
function cwToCard(it: ContinueWatchingEntry, onRemove?: (id: string) => void) {
  ...
  return {
    ...
    // Home rail: hides watched titles, so marking a card watched drops
    // it from view immediately (same idiom as the CW-remove).
    dropWhenWatched: true,
  };
}
...
export function HomeSection({ onNavigate }: HomeSectionProps) {
  ...
  // primary-key order.
  // unwatched: true — chino-api hides titles the user has finished and
  // backfills from later pages so each Home rail still returns up to
  // `limit` fresh titles. The hero (recentMovies[0]) and the Zap warm
  // pool (topRatedMovies[0]) draw from these rails, so flagging them
  // here also keeps the hero / warm candidate unwatched. Browse
  // (Movies/Series) and Search deliberately omit this so watched titles
  // stay findable for a rewatch.
  const recent = useItems(undefined, 20, 'movie', { sort: 'newest', unwatched: true });
  const recentSeries = useItems(undefined, 20, 'series', { sort: 'newest', unwatched: true });
  ...
  // App-start Zap warm: kick a lightweight byte-prefetch of the
  // DETERMINISTIC first Zap card so opening Zap from this cold home
  // screen is instant. We pass the TOP-RANKED candidate (top-rated
  // movie) and the hook stamps it with the fixed FIRST_CARD_RATIO seek
  // — the same deterministic card[0] the Zap screen stamps — so the
  // warmed init + seek-window segments are exactly what that card plays
  // first. Composes off the top-rated row we already fetch above — no
  // extra network, no coupling to useZapFeed. The hook is a session-once
  // no-op after the first warm and degrades to nothing when the item has
  // no usable duration. Falls back to the first recent movie when
  // top-rated hasn't resolved yet.
  useZapAppStartWarm(topRatedMovies[0] ?? recentMovies[0]);
  ...
  return (
    <>
      ...
      {/* Split chino-api's continue-watching feed into two shelves:
          ...
          Standard continue-watching home layout. Same MediaCard mapper for
          both rails — only the title + filter predicate differ. */}
