 ...
 * vector.
 *
 * Warming is two-layered. (a) Server prewarm: a debounced POST to
 * /prewarm primes the transcode pipeline for the next card (ffprobe /
 * OS page cache / NVENC context). (b) Client byte prefetch
 * (useZapPrefetch): for the next few queued cards we fetch() the same
 * master.m3u8 the player builds → resolve the per-device variant →
 * download the init + seek-window segments so the bytes are in the
 * browser cache before the swipe. The seek used for (b) mirrors
 * ZapCard's midpoint (server __zapSeekSec when present, else the
 * percent heuristic) so we warm bytes AROUND the seek point, not
 * segment 0 — avoiding the old ?t=60 mismatch that double-transcoded.
 */
export function ZapSection() {
  ...
  // The first card the screen shows gets a DETERMINISTIC seek stamped
  // onto it (FIRST_CARD_RATIO via stampFirstCardSeek) so it matches the
  // bytes the app-start home warm already fetched. ZapCard and the byte
  // prefetch both read __zapSeekSec verbatim, so stamping queue[0] is
  // the whole mechanism — card[0] warms-then-plays the identical
  // init+seek-window segments. Subsequent cards (index >= 1) keep their
  // existing per-mount random roll. No-op when queue[0] already carries
  // a server-rolled seek (warm pool).
  const queue = useMemo<KatalogItem[]>(() => {
    if (feed.queue.length === 0) return feed.queue;
    const head = stampFirstCardSeek(feed.queue[0]);
    if (head === feed.queue[0]) return feed.queue;
    return [head, ...feed.queue.slice(1)];
  }, [feed.queue]);
  ...
  // Client-side byte prefetch: as the feed yields cards and on every
  // settle, fetch() the next few cards' master.m3u8 → variant → init +
  // seek-window segments so the browser disk-caches them ahead of the
  // swipe. Complements (does not replace) the server prewarm POST
  // below: the server warms the pipeline, this downloads the bytes the
  // server warmed. The hook owns dedup-by-id, concurrency cap, LRU
  // budget, and aborts everything on unmount (leaving Zap).
  useZapPrefetch({ queue, activeId });
  ...
  const activeIndex = Math.max(0, queue.findIndex((it) => it.id === activeId));
