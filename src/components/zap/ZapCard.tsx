...
export function ZapCard({
  ...
  // seek point. Server warm-pool entries carry their own seekSec on
  // __zapSeekSec (chino-stream rolled the random and already pre-warmed
  // the relevant segments); when present we use it verbatim and skip
  // the client-side roll entirely so the cache hit lands on the right
  // bytes. The midSource string is validated against the closed union
  // so a misbehaving / future-versioned server can't poison the
  // downstream playUrl gate (which refuses 'fallback' sources).
  const midpoint = useMemo(() => {
    if (typeof item.__zapSeekSec === 'number' && item.__zapSeekSec > 0) {
      const allowed = ['segments', 'percent', 'fallback'] as const;
      type Allowed = (typeof allowed)[number];
      const raw = item.__zapMidSource;
      const source: Allowed = (allowed as readonly string[]).includes(raw ?? '')
        ? (raw as Allowed)
        : 'percent';
      return { seekSec: item.__zapSeekSec, source };
    }
    const durationMs = detail.data?.duration_ms ?? item.duration_ms;
    return pickZapMidpoint({ durationMs, segments, randomRatio });
  }, [item.__zapSeekSec, item.__zapMidSource, detail.data?.duration_ms, item.duration_ms, segments, randomRatio]);
