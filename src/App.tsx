...
//   /watchlist     the lists-aware watchlist surface (#110)
...
function pickRoute(path: string, search: string) {
  ...
  const personM = path.match(/^\/person\/([^/]+)\/?$/);
  if (personM) return <PersonPage personId={decodeURIComponent(personM[1])} />;

  if (path === '/me' || path === '/me/') return <ProfilePage />;

  // Deep-link straight into the Watchlist section inside the shell so
  // the sidebar / header stay present and a refresh / shared link lands
  // on the lists view.
  if (path === '/watchlist' || path === '/watchlist/') {
    return <ChinoApp initialSection="watchlist" />;
  }
